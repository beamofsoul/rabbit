...
/**
 * 将传入的日期时间数组(Array [1997, 7, 1])格式化为yyyy-MM-dd格式
 * @param date
 */
...
/**
 * 将传入的日期时间数组(Array [1997, 7, 1, 8, 30, 59])格式化为yyyy-MM-dd HH:mm:ss格式
 * @param datetime
 */
function formatDateTime (datetime) {
	let [date, hour, minute, second] = [formatDate(datetime), datetime[3], datetime[4], datetime[5]];
	if (hour < 10) {
		hour = "0" + hour;
	}
	if (minute < 10) {
