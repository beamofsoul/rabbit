// 当前用户能够操作的所有行为
var actions = {
    'delete': {'key': 'delete', 'url': 'delete', 'method': $.ideletey},
    'add': {'key': 'add', 'url': 'add', 'method': $.iposty},
    'update': {'key': 'update', 'url': 'update', 'method': $.iputy},
    'copy': {'key': 'copy', 'url': 'add', 'method': $.iposty}
};

var loadSingleDataUrl = 'single';
...
initUpdateForm = function (obj) {
	...
	if (showUpdateFormBefore) showUpdateFormBefore(getVueObject().vueUpdateForm, obj);
...
function submitDeleteForm() {
    ...
    $.when(submitForm(currentAction, ids, function (count) {
        if (count > 0) {
            toastSuccess('删除成功');
        } else {
        ...
        if(submitDeleteAfter) submitDeleteAfter({ids: ids, count: count});
		}, function (errorMessage) {
			// 由于global-ajax-exception-hanlder.js的存在，所以一旦出现异常不需要再次进行手动错误提示
//			toastError(errorMessage);
    })).done((data) => {
    	this.vueDeleteProgressVisible = false; //无论是否删除成功，都将删除进度条隐藏
    	this.vueDeleteModalVisible = false; //无论是否删除成功，都将确认删除提示框隐藏
    });
}
...
/**
 * 根据数据表格中复选框选中的记录Id，获取所对应的业务对象实例，并执行相应的回掉方法
 */
var getSingleData = function (instanceId, successCallbackBefore, successCallback, errorCallback) {
    var result = false;
    $.igety(loadSingleDataUrl + '/' + instanceId, null, data => {
        if ((successCallbackBefore && successCallbackBefore(data)) || !successCallbackBefore) {
        	if (successCallback) {
...
function validateFormRules(rule, value, callback) {
    ...
    if (rule.equal) {
    	...
    	var params = {'uniqueData' : value};
    	if (currentAction == actions.update) {
    		params.id = form.id;
    	}
        $.igety(rule.unique, params, isUnique => formValidateCallback(callback, isUnique))
    } else if (rule.activateOtherValidation) {
        var filedName = rule.activateOtherValidation;
        if (form[filedName] !== '') {
...
function submitForm(currentAction, data, callback, errorCallback) {
    ...
    let successCallback = returnData => fresh4NewData(returnData, () => callback(returnData));
    ...
    if (currentAction.key != actions.delete.key) {
    	...
    	data = clearNullStructureObject4JSON(formatNonstandardData2JSON(data));
    }
    currentAction.method(currentAction.url, data, successCallback, errorCallback);
}
...
function formatQueryFormData(vueBindFormQueryData) {
    let queryObj = {};
    for (let i in vueBindFormQueryData) {
    	/**
         * 2018-01-30
         * 增加判断: 如果queryObj中有属性值都为“”, 则不增加其为查询对象属性
         */
    	let data = vueBindFormQueryData[i]
    	if (data) {
    		queryObj[i] = '"' + data + '"';
    	}
    }
    return hasQueryForm && Object.keys(queryObj).length ? JSON.stringify(queryObj) : null;
}
