...
function formatTableData(data){
	...
	for(let i = 0; i < data.numberOfElements; i++) {
		value[i] = parseValuesOnTableEachRow(data.content[i]);
	}
	return value;
}
...
function doLoadPage () {
	let _self = this;
	const loadingMessage = toastLoading('正在加载中...',0);
	const loadingTime = 120;
	clearDataValue(currentCheckedTableRowName); // 清空选中数据
	
	/**
	 * 装配请求参数: page - 第几页, size - 每页几条数据
	 * 如果存在table数据的复合查询条件, 将复合查询条件格式化后装载进请求参数对象
	 */
	let params = {page: (_self[currentCurrentPageName]-1) , size: _self[currentPageSizeName]};
	let conditions = formatQueryFormData(_self[currentQueryFormName]);
	if (conditions) {
	...
	$.when($.igety(
			...
			data => {
				...
				_self[currentRecordTotalName] = data.totalElements; // 总记录数
			},
			errorMessage => _self.$Message.error(errorMessage)
	)).done((data) => setTimeout(loadingMessage, loadingTime)); // 销毁加载提示
}
////////////////////////////////////////////////
/**
